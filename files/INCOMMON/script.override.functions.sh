...

{
        ...
        if [ "${selfsigned}" = "n" ]; then
                ${Echo} "Put the certificate from ICS in the file: ${certpath}server.crt" >> ${messages}
...

{
			...
			# Notes
			# 1. InCommon does not have access to TCS CA's, nor needs them. Elements have been commented out, but kept here for reference.
